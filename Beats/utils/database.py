...
def addFavorite(userID, favType, entry, entryID):
    ...
    entry = entry + '~~' + entryID
    if len(oldFavs) > 0:
        newFavs = oldFavs[0] + "::" + entry 
    else:
        newFavs = entry
    cmd = "UPDATE UserInfo SET fav%ss='%s' WHERE UserID = %d;"%(favType, newFavs, userID)
    c.execute(cmd)
    db.commit()
    db.close()
...
def rmFavorite(userID, favType, entry, entryID):
    ...
    cmd = "SELECT fav%ss FROM UserInfo WHERE UserID = %d;"%(favType, userID)
    oldFavs = c.execute(cmd).fetchone() #is the string
    splitFavs = oldFavs[0].split('::')
    splitFavs = [i for i in splitFavs if i != (entry + '~~' + entryID)]
